...
describe('client factory bundle', () => {
  ...
  it('list slots register with id, settings card with key', () => {
    ...
    expect(source).toContain("wide ? 'dsh-poker' : 'dsh'")
    expect(source).not.toContain("'斗地主'")
    ...
    expect(source).toContain('data-doudizhu-tab')
    expect(source).not.toContain("location.hash = '#/doudizhu'")
