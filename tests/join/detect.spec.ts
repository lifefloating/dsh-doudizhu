...
describe('invite gate copy', () => {
  it('points at the official DSH site and this plugin repo', () => {
    ...
    expect(gate).toContain('<SeatTags role="landlord" />')
    expect(gate).toContain('<SeatTags role="farmer" />')
    expect(gate).not.toMatch(/<figcaption>地主<\/figcaption>/)
    expect(gate).not.toMatch(/<figcaption>农民<\/figcaption>/)
